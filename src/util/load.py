...
def load_BFM_UVspace(model_path):
    ...
    model_uv_patch = np.load('data/uv_data/BFM_UVspace_patch.npy', allow_pickle=True).item()

    model['shapeMU'] = model_uv_patch['shapeMU_uv'].astype(np.float32)
    model['shapePC'] = model_uv_patch['shapePC_uv'].astype(np.float32)
    model['shapeEV'] = model['shapeEV'].astype(np.float32)
    model['expEV'] = model['expEV'].astype(np.float32)
    model['expPC'] = model_uv_patch['expPC_uv'].astype(np.float32)
    ...
    # kpt ind
    model['kpt_ind'] = model_uv_patch['kpt_ind_uv'].astype(np.int32)
